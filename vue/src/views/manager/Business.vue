    ...
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="timeRange" label="营业时间"></el-table-column>
                <el-table-column prop="paycode" label="支付二维码">
          <template v-slot="scope">
            <div style="display: flex; align-items: center">
              <el-image style="width: 40px; height: 40px;" v-if="scope.row.paycode"
                        :src="scope.row.paycode" :preview-src-list="[scope.row.paycode]"></el-image>
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog title="商家" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        ...
        <el-form-item label="支付二维码">
              ...
              :headers="{ token: user.token }"
              list-type="picture"
              :on-success="handpayCodeSuccess"
          >
            <el-button type="primary">上传支付二维码</el-button>
          </el-upload>
  ...
  methods: {
    ...
    handleLicenseSuccess(response, file, fileList) {
      this.form.license = response.data
       ...
       handpayCodeSuccess(response, file, fileList) {
      this.form.paycode = response.data
    }
  }
}
</script>
