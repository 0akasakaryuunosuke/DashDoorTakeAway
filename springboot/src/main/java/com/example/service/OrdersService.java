...
import com.example.common.config.OrderWebSocketHandler;
...
public void updateById(Orders orders) {
    ...
    if (OrderStatusEnum.NO_SEND.getValue().equals(orders.getStatus())) {
      ...
      ordersDTO.setBusinessId(orders.getBusinessId());
      ...
      ordersDTO.setComment(orders.getComment());
       ordersDTO.setPayType(orders.getPayType());
        OrderWebSocketHandler.sendOrderAlertToBusiness( ordersDTO);
    ...
    public List<Orders> selectAll(Orders orders) {
        ...
        if (RoleEnum.USER.name().equals(role)) {  // 如果是用户的话   只能查询自己的分类
            orders.setUserId(currentUser.getId());  // 设置用户自己的Id作为查询条件
        }
